...
open(20,file=trim(ADJUSTL(dir_entrada))//secao_arq,status="unknown",access="direct",&
& form="unformatted",recl=4*Nx*Nt) !Abre o arquivo
read(20,rec=1)((sismo(i,j),i=1,Nt),j=1,Nx) !Armazena o modelo de velocidade em vel
close(20)
...
do k=Nt,1,-1 !Inicio do loop da marcha no tempo
    
  !imprime o andamento da marcha no tempo na tela
  if(mod(k,50).eq.0) write(*,*)"tempo = ",k

  !Aplicacao do termo fonte
!  if(k.le.Nf) p1(izf,ixf) = p1(izf,ixf) - fon_d2g((k-1)*Dt-Tf,fc,famp)

  do j=1,Nx
    p1(iprof,j) = p1(iprof,j) + sismo(k,j)
  enddo
